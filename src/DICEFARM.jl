...
function initialize_dice_farm(p, start_year, end_year, start_dice_year, TCR, ECS)
    ...
    #--------------------------------------------------------------------------------------------------------------------
    # Carry out some data prep.
    #--------------------------------------------------------------------------------------------------------------------

    # Get an instance of DICE2016 to access some data.
    dice2016 = MimiDICE2016.get_model()
    run(dice2016)

    # Get DICE2016 CO₂ emissions in 2015 and convert units to GtC.
    ...
    # Pad annualized paramters so the have the time length of the full DICE-FARM model, not juts DICE.
    ...
    rcp_total_co2   = m[:co2_cycle, :E]                # Total FAIR CO₂ emissions = fossil + landuse (GtC).
    rcp_fossil_ch4  = m[:ch4_cycle, :fossil_emiss_CH₄] # Fossil sources of methane (Mt CH₄/yr).
    rcp_fossil_n2o  = m[:n2o_cycle, :fossil_emiss_N₂O] # Fossil sources of nitrous oxide (Mt N/yr).

    # Calculate RCP index to begin interpolation so there's a smooth transition from historic RCP to present day DICE CO₂ emissions.
